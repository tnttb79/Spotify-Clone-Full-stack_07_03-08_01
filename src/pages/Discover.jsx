    ...
    <div className='flex flex-col'>
      <div className='w-full flex flex-col sm:flex-row justify-between items-center mt-4 mb-10'>
        <h2 className='font-bold text-3xl text-white text-left'>
          {genres.find((g) => g.value === genre).title}
        </h2>
        <select
      ...
      <div className='flex flex-wrap m:justify-start justify-center gap-8'>
        {data?.data?.map((track, i) => (
          <SongCard
            img={
              track.attributes?.artwork?.url?.replace("{w}x{h}", "500x500") ||
            ...
            track={{
              ...
              images: {
                coverart: track.attributes?.artwork?.url?.replace(
                  "{w}x{h}",
                  "500x500"
                ),
              },
              hub: {
                actions: [
                  {},
                  {
                    uri: track.attributes?.previews?.[0]?.url,
                  },
                ],
              },
            }}
